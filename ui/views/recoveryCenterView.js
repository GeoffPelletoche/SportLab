  ...
  return `
    <div class="recovery-center sl-stack-lg">
      <section class="sl-card recovery-hero">
        <div><p class="sl-eyebrow">SportLab V11.6.0 · Safe Recovery</p><h2>🛟 Cloud Recovery & Conflict Center</h2><p class="sl-muted">Compare, protège et restaure les données SportLab sans modifier les moteurs métier.</p></div>
      ...
      <section class="recovery-actions-grid">
        ...
        ${actionCard("🔀", "Fusion sécurisée", "Synchronise sans arbitrage silencieux : toute divergence ambiguë reste en conflit.", "mergeSportLabCloudData()", "Fusionner")}
...
function renderConflict(item){const pending=item.status==="pending"||item.winner==="pending";return `<div class="recovery-row"><div><strong>${escapeHtml(item.namespace)} · ${escapeHtml(item.key)}</strong><small>${date(item.at)}${pending ? " · divergence protégée" : ""}</small></div><span class="sl-badge ${pending ? "sl-badge-warning" : "sl-badge-neutral"}">${escapeHtml(pending ? "Décision requise" : item.winner === "server" ? "Cloud retenu" : item.winner === "client" ? "Local retenu" : "Résolu")}</span></div>`;}
