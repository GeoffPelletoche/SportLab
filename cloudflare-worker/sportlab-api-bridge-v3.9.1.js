...
async function handleFootballTeamFixtures(url, env) {
  ...
  const cacheKey =
    `football:v9:team-fixtures:${team}:${league || "all"}:${season}:${limit}`;
  ...
  for (const seasonToFetch of seasons) {
    ...
    for (const fixture of fixtures) {
      const fixtureId =
        fixture.fixture?.id ||
        fixture.id ||
        `${seasonToFetch}-${fixture.fixture?.date || fixture.date}-${fixture.teams?.home?.id}-${fixture.teams?.away?.id}`;

      if (knownIds.has(String(fixtureId))) {
        continue;
      }
    ...
    const finishedCount =
      collectedFixtures.filter(fixture =>
        isFinishedFootballStatus(
...
async function handleRugbyTeamGames(url, env) {
  ...
  const cacheKey =
    `rugby:v10:team-games:${team}:${normalizeTeamName(teamName || "unknown")}:${league || "all"}:${season}:${limit}`;
 ...
 * ================================================================
 */
function normalizeFootballTeamFixtures(
  items,
  teamId
) {
  return items.map(item => {
    ...
    const homeName = item.teams?.home?.name || null;
    const awayName = item.teams?.away?.name || null;

    const isHome =
      String(homeId) === String(teamId) ||
      (
        normalizedTargetName &&
        normalizeTeamName(homeName) === normalizedTargetName
      );
 ...
 * ================================================================
 */
function normalizeRugbyTeamGames(
  items,
  teamId,
  teamName = ""
) {
