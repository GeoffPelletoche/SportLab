...
// V11.3.10 / Bridge 3.10.1 — limiteur conservateur API-Sports.
// PRO Football autorise davantage, mais SportLab garde volontairement une marge
// pour éviter les rafales de requêtes lors du chargement des historiques.
const API_SPORTS_MIN_INTERVAL_MS = 275; // ~3,6 requêtes/seconde
const API_SPORTS_MINUTE_BUDGET = 240; // marge sous la limite contractuelle
const API_SPORTS_RATE_RETRIES = 2;
let apiSportsQueue = Promise.resolve();
let apiSportsLastStartAt = 0;
const apiSportsStarts = [];

class ApiSportsBridgeError extends Error {
  constructor(message, { code = "API_SPORTS_ERROR", status = 502, retryAfterMs = 0, details = null } = {}) {
    super(message);
    this.name = "ApiSportsBridgeError";
    this.code = code;
    this.status = status;
...
function scheduleApiSportsFetch(task) {
  const execute = async () => {
    const now = Date.now();
    while (apiSportsStarts.length && apiSportsStarts[0] <= now - 60_000) apiSportsStarts.shift();

    const intervalWait = Math.max(0, API_SPORTS_MIN_INTERVAL_MS - (Date.now() - apiSportsLastStartAt));
    if (intervalWait) await wait(intervalWait);

    const current = Date.now();
    while (apiSportsStarts.length && apiSportsStarts[0] <= current - 60_000) apiSportsStarts.shift();
    if (apiSportsStarts.length >= API_SPORTS_MINUTE_BUDGET) {
      const minuteWait = Math.max(250, 60_000 - (current - apiSportsStarts[0]) + 50);
      await wait(minuteWait);
    }

    const startedAt = Date.now();
    apiSportsLastStartAt = startedAt;
    apiSportsStarts.push(startedAt);
    return task();
  };
  const scheduled = apiSportsQueue.then(execute, execute);
  apiSportsQueue = scheduled.catch(() => {});
  return scheduled;
}

function extractApiSportsError(data) {
  const errors = data?.errors;
  if (!errors) return null;
  if (Array.isArray(errors) && !errors.length) return null;
  if (typeof errors === "object" && !Array.isArray(errors) && !Object.keys(errors).length) return null;
  if (typeof errors === "string" && !errors.trim()) return null;

  const serialized = typeof errors === "string" ? errors : JSON.stringify(errors);
  const rateLimited = Boolean(errors?.rateLimit) || /too many requests|requests per minute|rate.?limit/i.test(serialized);
  if (rateLimited) {
    return new ApiSportsBridgeError(
      "API-Sports: limite temporaire de requêtes par minute atteinte.",
      { code: "API_SPORTS_RATE_LIMIT", status: 429, retryAfterMs: 1500, details: errors }
    );
  }

  const subscription = /subscription|plan|access denied|forbidden/i.test(serialized);
  if (subscription) {
    return new ApiSportsBridgeError(
      `API-Sports: accès non autorisé pour cette ressource. ${serialized}`,
      { code: "API_SPORTS_SUBSCRIPTION", status: 403, details: errors }
    );
  }

  return new ApiSportsBridgeError(
    `API-Sports error: ${serialized}`,
    { code: "API_SPORTS_ERROR", status: 502, details: errors }
  );
}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      /*
       * NFL — Sprint 0.1 Data Integration.
       */
      if (path === "/nfl/games") {
        return await handleNflGames(url, env);
      }
      if (path === "/nfl/team-games") {
        return await handleNflTeamGames(url, env);
      }
      if (path === "/nfl/game-result") {
        return await handleNflGameResult(url, env);
      }
      ...
      const code = String(error?.code || "WORKER_ERROR");
      return corsResponse(
        {
          error: code,
          code,
...
async function handleFootballFixtures(url, env) {
  ...
  const explicitSeason = url.searchParams.get("season");

  if (!league || !from || !to) {
    return corsResponse({ error: "MISSING_PARAMS", required: ["league", "from", "to"] }, 400);
  }

  const season = explicitSeason || await getCurrentFootballSeason(env, league);

  if (!season) {
    return corsResponse({
      error: "NO_FOOTBALL_SEASON",
      message: "Aucune saison football exploitable n'a été détectée.",
      league, from, to
    }, 422);
  }

  const cacheKey = `football:v10.1:fixtures:${league}:${season}:${from}:${to}`;
  ...
  const normalized = normalizeFootballFixtures(rawFixtures, league, season);

  if (normalized.length > 0) {
    await setCache(env, cacheKey, normalized);
  } else {
    await setCacheWithTtl(env, cacheKey, normalized, 60 * 15);
  }
  ...
  return corsResponse({
    ...
    league, season, from, to,
    rawResults: data.results ?? rawFixtures.length,
...
/** ================================================================
 * NFL — Sprint 0.1 Data Integration
 * API-NFL league 1 = NFL. The API season is the year the season starts.
 * ================================================================ */
async function handleNflGames(url, env) {
  const league = Number(url.searchParams.get("league") || 1);
  const from = url.searchParams.get("from");
  const to = url.searchParams.get("to") || from;
  if (!from) return corsResponse({ error: "NFL_FROM_REQUIRED", message: "Paramètre from requis (YYYY-MM-DD)." }, 400);
  const season = await detectNflSeason(env);
  const cacheKey = `nfl:v1:games:${league}:${season}:${from}:${to}`;
  const cached = await getCache(env, cacheKey);
  if (cached) return corsResponse({ ...cached, source: "cache" });
  const days = enumerateIsoDates(from, to, 8);
  const rows = [];
  for (const date of days) {
    const data = await fetchApiSports(`https://v1.american-football.api-sports.io/games?league=${encodeURIComponent(league)}&season=${encodeURIComponent(season)}&date=${encodeURIComponent(date)}`, env);
    rows.push(...(Array.isArray(data?.response) ? data.response : []));
  }
  const response = rows.filter(isOfficialNflGame).map(normalizeNflGame);
  const payload = { response, season, results: response.length, source: "api" };
  await setCacheWithTtl(env, cacheKey, payload, 15 * 60);
  return corsResponse(payload);
}

async function handleNflTeamGames(url, env) {
  const team = Number(url.searchParams.get("team") || 0);
  const league = Number(url.searchParams.get("league") || 1);
  const requestedSeason = Number(url.searchParams.get("season") || 0);
  const limit = Math.min(MAX_HISTORY_LIMIT, Math.max(1, Number(url.searchParams.get("limit") || DEFAULT_HISTORY_LIMIT)));
  if (!team) return corsResponse({ error: "NFL_TEAM_REQUIRED", message: "Paramètre team requis." }, 400);
  const currentSeason = requestedSeason || await detectNflSeason(env);
  const cacheKey = `nfl:v1:team-games:${team}:${league}:${currentSeason}:${limit}`;
  const cached = await getCache(env, cacheKey);
  if (cached?.response?.length) return corsResponse({ ...cached, source: "cache" });
  const collected = [];
  const seen = new Set();
  for (let offset = 0; offset < HISTORY_SEASONS_DEPTH && collected.length < limit; offset += 1) {
    const season = currentSeason - offset;
    const data = await fetchApiSports(`https://v1.american-football.api-sports.io/games?league=${encodeURIComponent(league)}&season=${encodeURIComponent(season)}&team=${encodeURIComponent(team)}`, env);
    for (const raw of Array.isArray(data?.response) ? data.response : []) {
      if (!isOfficialNflGame(raw) || !isFinishedNflGame(raw)) continue;
      const game = normalizeNflHistoryGame(raw, team);
      if (!game.id || seen.has(game.id)) continue;
      seen.add(game.id); collected.push(game);
    }
  }
  collected.sort((a,b) => Date.parse(b.date||0)-Date.parse(a.date||0));
  const response = collected.slice(0, limit).map((item,index) => ({ ...item, recencyWeight: Math.max(MIN_RECENCY_WEIGHT, 1 - index * RECENCY_WEIGHT_STEP) }));
  const payload = { response, season: currentSeason, results: response.length, source: "api" };
  if (response.length) await setCacheWithTtl(env, cacheKey, payload, 6 * 60 * 60);
  return corsResponse(payload);
}

async function handleNflGameResult(url, env) {
  const id = Number(url.searchParams.get("id") || 0);
  if (!id) return corsResponse({ error: "NFL_GAME_ID_REQUIRED" }, 400);
  const data = await fetchApiSports(`https://v1.american-football.api-sports.io/games?id=${encodeURIComponent(id)}`, env);
  const raw = Array.isArray(data?.response) ? data.response[0] : null;
  return corsResponse({ response: raw ? normalizeNflGame(raw) : null, source: "api" });
}

async function detectNflSeason(env) {
  const cacheKey = "nfl:v1:current-season:league:1";
  const cached = await getCache(env, cacheKey);
  if (cached?.season) return Number(cached.season);
  const data = await fetchApiSports("https://v1.american-football.api-sports.io/leagues?id=1&current=true", env);
  const league = Array.isArray(data?.response) ? data.response[0] : null;
  const seasons = Array.isArray(league?.seasons) ? league.seasons : [];
  const current = seasons.find(item => item?.current) || seasons[seasons.length - 1];
  const season = Number(current?.year || new Date().getUTCFullYear());
  await setCacheWithTtl(env, cacheKey, { season }, 12 * 60 * 60);
  return season;
}
function normalizeNflGame(item) {
  const timestamp = Number(item?.game?.date?.timestamp || 0);
  return { id: item?.game?.id || null, date: timestamp ? new Date(timestamp*1000).toISOString() : [item?.game?.date?.date,item?.game?.date?.time].filter(Boolean).join("T"), status: item?.game?.status?.short || item?.game?.status?.long || null, stage: item?.game?.stage || null, week: item?.game?.week || null, leagueId: item?.league?.id || 1, season: Number(item?.league?.season || 0) || null, homeId: item?.teams?.home?.id || null, awayId: item?.teams?.away?.id || null, home: item?.teams?.home?.name || "", away: item?.teams?.away?.name || "", homeLogo: item?.teams?.home?.logo || "", awayLogo: item?.teams?.away?.logo || "", homeScore: finiteOrNull(item?.scores?.home?.total), awayScore: finiteOrNull(item?.scores?.away?.total) };
}
function normalizeNflHistoryGame(item, teamId) {
  const game = normalizeNflGame(item); const isHome = Number(game.homeId) === Number(teamId);
  return { ...game, teamId, opponentId: isHome ? game.awayId : game.homeId, opponent: isHome ? game.away : game.home, isHome, pointsFor: isHome ? game.homeScore : game.awayScore, pointsAgainst: isHome ? game.awayScore : game.homeScore, totalPoints: Number(game.homeScore || 0) + Number(game.awayScore || 0) };
}
function isFinishedNflGame(item) { return ["FT","AOT"].includes(String(item?.game?.status?.short || "").toUpperCase()); }
function isOfficialNflGame(item) { return !/pre.?season/i.test(String(item?.game?.stage || "")); }
function finiteOrNull(value) { const n=Number(value); return Number.isFinite(n) ? n : null; }
function enumerateIsoDates(from, to, maxDays=8) { const start=new Date(`${from}T00:00:00Z`), end=new Date(`${to}T00:00:00Z`); if(Number.isNaN(start.getTime())||Number.isNaN(end.getTime())||end<start) return [from]; const out=[]; for(let d=new Date(start); d<=end && out.length<maxDays; d.setUTCDate(d.getUTCDate()+1)) out.push(d.toISOString().slice(0,10)); return out; }
...
async function fetchApiSports(apiUrl, env) {
  if (!env.API_SPORTS_KEY) {
    throw new ApiSportsBridgeError(
      "Missing API_SPORTS_KEY secret",
      { code: "API_SPORTS_KEY_MISSING", status: 500 }
    );
  }

  let lastRateError = null;
  for (let attempt = 0; attempt <= API_SPORTS_RATE_RETRIES; attempt += 1) {
    if (attempt > 0) {
      const retryDelay = Math.max(Number(lastRateError?.retryAfterMs || 0), 1200 * attempt);
      await wait(retryDelay);
    }

    const response = await scheduleApiSportsFetch(() => fetch(apiUrl, {
    ...
    try {
      ...
      throw new ApiSportsBridgeError(
        ...
        { code: "API_SPORTS_INVALID_JSON", status: 502 }
      );
    }

    const businessError = extractApiSportsError(data);
    if (response.status === 429 || businessError?.code === "API_SPORTS_RATE_LIMIT") {
      const headerSeconds = Number(response.headers?.get?.("retry-after") || 0);
      lastRateError = businessError || new ApiSportsBridgeError(
        "API-Sports: limite temporaire de requêtes atteinte.",
        { code: "API_SPORTS_RATE_LIMIT", status: 429, retryAfterMs: headerSeconds > 0 ? headerSeconds * 1000 : 1500, details: data }
      );
      if (headerSeconds > 0) lastRateError.retryAfterMs = Math.max(lastRateError.retryAfterMs || 0, headerSeconds * 1000);
      if (attempt < API_SPORTS_RATE_RETRIES) continue;
      throw lastRateError;
    }

    if (!response.ok) {
      const status = Number(response.status || 502);
      const code = status === 401 ? "API_SPORTS_UNAUTHORIZED" : status === 403 ? "API_SPORTS_SUBSCRIPTION" : "API_SPORTS_HTTP_ERROR";
      throw new ApiSportsBridgeError(
        `API-Sports HTTP ${status}: ${JSON.stringify(data)}`,
        { code, status, details: data }
      );
    }
  ...
  throw lastRateError || new ApiSportsBridgeError("API-Sports indisponible.");
}
