...
async function handleFootballTeamFixtures(url, env) {
  ...
  const cacheKey =
    `football:v8:team-fixtures:${team}:${league || "all"}:${season}:${limit}`;
...
async function handleRugbyTeamGames(url, env) {
  ...
  const cacheKey =
    `rugby:v8:team-games:${team}:${league || "all"}:${season}:${limit}`;
...
async function fetchRugbyTeamGamesRaw(
  ...
  for (const seasonToFetch of seasons) {
    ...
    for (const apiUrl of urls) {
      ...
      /*
       * Si la requête par ligue a trouvé des matchs,
       * il n’est pas nécessaire d’interroger une deuxième fois
       * la même saison par équipe.
       */
      if (games.length > 0) {
        break;
      }
