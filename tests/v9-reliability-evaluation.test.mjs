...
test("V9 worker fixes rugby team fallback", async () => { const code = await readFile(new URL("cloudflare-worker/sportlab-api-bridge-v3.10.0.js", root), "utf8"); assert.match(code, /matchedForTeam > 0/); assert.match(code, /football\/game-result/); });
